 ...
 *
 * Usage: patch-openclaw-slack-deny-feedback.js <search-root> [<search-root>...]
...
if (roots.length === 0) {
  console.error("Usage: patch-openclaw-slack-deny-feedback.js <search-root> [<search-root>...]");
  process.exit(2);
}
...
function findSlackPackageRoots(searchRoots) {
  const found = new Set();
...
function locatePrepareModule(packageRoot) {
...
function buildHelperSource() {
  return [
    ...
    '\t\tctx?.logger?.warn?.({ err: ephemeralError, channel }, "nemoclaw: slack denial ephemeral feedback failed; falling back to DM (#4752)");',
    "\t}",
    "\ttry {",
...
function patchPrepareModule(file) {
...
for (const packageRoot of packageRoots) {
  ...
  const patched = fs.readFileSync(prepareFile, "utf8");
...
console.log(
  `INFO: patched OpenClaw Slack denial feedback in ${patchedFiles.map((file) => path.relative(process.cwd(), file)).join(", ")}`,
);
