...
const PATCH_SCRIPT = path.join(
  ...
  "..",
  "scripts",
  "patch-openclaw-slack-deny-feedback.js",
);
...
function runPatch(...roots: string[]) {
  return spawnSync(process.execPath, [PATCH_SCRIPT, ...roots], {
    encoding: "utf-8",
    timeout: 10000,
  });
}
  ...
  const client = {
    chat: {
      postEphemeral: async (payload: Omit<FeedbackCall, "method">) => {
        ...
        if (params.ephemeralFails) throw new Error("not_allowed");
        return { ok: true };
      },
...
describe("OpenClaw Slack denial-feedback patch", () => {
  it("injects bounded sender feedback while keeping the command denied", async () => {
    ...
    try {
      ...
      // Ephemeral failure falls back to a DM postMessage.
