...
export function acquireLock(): void {
  ...
  for (let i = 0; i < LOCK_MAX_RETRIES; i++) {
    try {
      ...
      try {
        lockMtimeMs = fs.statSync(LOCK_DIR).mtimeMs;
      } catch {
      ...
      if (decision === "break") {
        // Re-read the owner just before breaking so a lock another process
        // already replaced with its own is not clobbered.
        let stillSameOwner = true;
        if (ownerPid !== null) {
          try {
            const recheck = Number.parseInt(fs.readFileSync(LOCK_OWNER, "utf-8").trim(), 10);
            stillSameOwner = recheck === ownerPid;
          } catch {
            stillSameOwner = true;
          }
        }
