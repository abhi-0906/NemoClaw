...
const REVIEWED_OPENCLAW_2026_5_27_MANAGED_PROXY_PATCHED_CLAUSE =
  'process.env.OPENSHELL_SANDBOX === "1" && !params.dispatcherPolicy';
...
if (!blocked) throw new Error('private IP literal was not blocked');`,
  ...
  it("anchors the managed-proxy strict-fetch gate to the reviewed OpenClaw 2026.5.27 shape", () => {
    // Fail loudly if the Dockerfile Patch 4 target/replacement drifts from the
    // reviewed dist shape, so a future OpenClaw bump cannot silently skip the
    // Google Chat cert-fetch proxy routing (#4687).
    const dockerfile = fs.readFileSync(DOCKERFILE, "utf-8");
    expect(dockerfile).toContain(REVIEWED_OPENCLAW_2026_5_27_MANAGED_PROXY_SHAPE);
    expect(dockerfile).toContain(REVIEWED_OPENCLAW_2026_5_27_MANAGED_PROXY_PATCHED_CLAUSE);
    expect(dockerfile).toContain("nemoclaw: route unconfigured strict fetch");
  });
